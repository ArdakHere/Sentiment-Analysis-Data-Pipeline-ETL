...
def getGuardianNews(url: str):
    ...
    clean_text = {
        ...
        for link in links
        if not any(substring in link['aria-label'] for substring in unwanted_substrings if len(html.unescape(link['aria-label'])) >= 33)
    }
...
def getBBCnews(url: str):
    ...
    clean_texts = {html.unescape(tag.get_text()) for tag in h2_tags if len(html.unescape(tag.get_text())) >= 33}
...
#         and len(html.unescape(link['aria-label'])) >= 15
#     ]
#     return clean_texts
...
def getNYtimesnews(url):
    ...
    clean_texts_summary = { html.unescape(p.get_text()) for p in soup.find_all('p', class_=regex_pattern_summary)
                           if len(html.unescape(p.get_text())) >= 33 }
    clean_texts_indicate = { html.unescape(p.get_text()) for p in soup.find_all('p', class_=regex_pattern_indicate)
                            if len(html.unescape(p.get_text())) >= 33 }

    clean_texts_total = clean_texts_indicate.union(clean_texts_summary)

    clean_texts_total = list(clean_texts_total)
